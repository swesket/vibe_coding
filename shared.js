...
const i18n = {
  ko: {
    ...
    guide: {
      ...
      items: [
        { icon: "🇰🇷", title: "국내 셀렉트샵 공략법", body: "무신사, 29CM, W컨셉 등 국내 셀렉트샵은 시즌 세일 시기를 노리는 것이 핵심입니다. 보통 1월과 7월에 대규모 시즌 오프 세일이 진행되며, 이 시기에는 인기 브랜드 상품도 30~70%까지 할인됩니다. 앱 전용 쿠폰이 웹보다 많은 경우가 대부분이므로 반드시 모바일 앱을 설치하고 알림을 켜두세요. 특히 무신사는 매일 변경되는 타임 세일과 랭킹 기반 추천 시스템을 활용하면 트렌드 파악과 동시에 합리적인 가격에 구매할 수 있습니다. 29CM은 에디터 픽 코너를 운영하여 스타일링 제안과 함께 상품을 소개하므로, 코디 아이디어가 필요한 분들에게 특히 유용합니다. 또한 각 플랫폼의 멤버십 등급 제도를 적극 활용하는 것이 좋습니다. 무신사의 경우 구매 금액에 따라 등급이 올라가며, 상위 등급에서는 추가 할인 쿠폰과 무료 배송 혜택이 제공됩니다. 29CM의 멤버십 역시 등급별 적립률이 달라지므로 하나의 플랫폼에 구매를 집중하는 것이 장기적으로 유리합니다. 사이즈 선택에 실패하지 않으려면 각 플랫폼의 실측 정보와 리뷰 사진을 꼼꼼히 확인하세요. 무신사는 체형별 리뷰 필터 기능이 있어 자신과 비슷한 체형의 리뷰만 골라볼 수 있습니다. 반품 정책도 플랫폼마다 다르니 구매 전에 확인하는 습관을 들이세요. 무료 반품을 지원하는 곳이 많지만, 일부 브랜드나 세일 상품은 반품이 제한될 수 있습니다. 마지막으로, 계절이 바뀌기 직전에 다음 시즌 아이템을 미리 구매하는 전략도 추천합니다. 시즌 초반에는 신상품 할인 쿠폰이 자주 발행되고, 아직 사이즈가 풍부하기 때문에 원하는 아이템을 확보하기 훨씬 수월합니다.", tip: "TIP: 첫 구매 쿠폰은 반드시 챙기세요. 대부분 10~15% 할인을 제공합니다." },
        { icon: "💎", title: "럭셔리 온라인 쇼핑 전략", body: "해외 럭셔리 플랫폼 구매 시 관세(물품가 15만원 초과 시)와 부가세를 반드시 계산해야 합니다. 의류의 경우 관세율이 약 13%, 여기에 부가세 10%가 더해지므로 실제 추가 비용이 상당할 수 있습니다. SSENSE와 Mytheresa는 시즌 오프 세일 때 최대 70%까지 할인하며, 특히 SSENSE는 관부가세를 결제 시 미리 포함하는 DDP 방식이라 수령 시 추가 비용이 없어 편리합니다. Farfetch는 전 세계 수백 개 부티크의 재고를 한 곳에서 비교할 수 있어 다른 플랫폼에서 품절된 상품도 찾기 쉽고, 독점 캡슐 컬렉션도 종종 출시합니다. 환율이 유리한 시점을 노려 구매하면 추가 절약이 가능한데, 원/달러 환율이 50원만 달라져도 고가 아이템에서는 수만 원 차이가 발생합니다. 해외 카드 결제 수수료도 비교해볼 필요가 있습니다. 일부 카드사는 해외 결제 수수료 면제 혜택이나 캐시백을 제공하므로, 럭셔리 직구 전용 카드를 하나 만들어두면 장기적으로 절약됩니다. Net-a-Porter, MatchesFashion 같은 플랫폼은 시즌 중에도 프라이빗 세일을 진행하니 뉴스레터를 구독해두세요. 사이즈 선택이 어려울 경우 국내 백화점에서 같은 브랜드를 미리 피팅해보는 것이 가장 확실합니다. 특히 유럽 브랜드는 한국 사이즈와 체계가 다르므로 사이즈 차트를 꼼꼼히 비교하세요. 반품 정책은 플랫폼마다 크게 다릅니다. SSENSE는 30일 이내 무료 반품을 지원하지만, 일부 플랫폼은 국제 반품 배송비를 고객이 부담해야 합니다. 또한 새 시즌 컬렉션 출시 직후보다는 시즌 중반 이후에 구매하면 할인 확률이 높아지므로, 긴급하지 않다면 인내심을 갖고 기다리는 것이 현명합니다. 위시리스트 기능을 활용하여 원하는 아이템의 가격 변동을 추적하는 것도 좋은 전략입니다.", tip: "TIP: 해외 직구 시 개인통관고유부호를 미리 발급받아두세요." },
        { icon: "🛹", title: "스트릿웨어 드롭 대응법", body: "Supreme, Palace, Stussy 같은 스트릿 브랜드는 매주 정해진 시간에 신제품을 '드롭' 방식으로 판매합니다. 드롭 성공률을 높이려면 체계적인 준비가 필수입니다. 먼저 인스타그램이나 트위터에서 리크(leak) 계정을 팔로우하여 다음 주 발매 아이템을 미리 파악하세요. Dropsbyjay, Supreme Leaks 같은 계정이 대표적입니다. 발매 아이템 목록을 확인했다면, 사이즈와 컬러를 미리 확정해 두세요. 드롭 시간에는 1~2분 안에 결제까지 완료해야 하므로 고민할 여유가 없습니다. 결제 정보와 배송 주소는 반드시 사전에 저장해두고, 가능하면 자동 완성 기능을 설정해 체크아웃 시간을 단축하세요. 인기 아이템은 30초 내에 매진되는 경우도 흔합니다. Kith는 자체 디자인 제품과 함께 나이키, 뉴발란스 등과의 콜라보 제품을 정기적으로 발매하며, 이러한 콜라보 제품은 리셀 시장에서 프리미엄이 붙는 경향이 강합니다. Beams 역시 일본 특유의 감성을 살린 독점 콜라보를 자주 진행합니다. 드롭에 실패했다면 리셀 시장을 바로 확인하기보다는 1~2주 기다리는 것이 좋습니다. 초기 리셀가는 과열되어 있는 경우가 많고, 시간이 지나면 가격이 안정되는 패턴이 일반적입니다. 또한 해외 드롭과 국내 드롭의 시간대가 다를 수 있으니 시차를 확인하세요. 모바일보다 데스크톱에서 체크아웃 속도가 빠른 경우가 많으며, 안정적인 인터넷 환경에서 시도하는 것이 성공률을 높이는 데 도움이 됩니다. 마지막으로, 같은 브랜드의 매장에서 직접 줄을 서는 오프라인 드롭도 고려해볼 만합니다. 온라인보다 경쟁이 덜한 경우도 있으며, 현장에서만 만날 수 있는 한정 아이템이 나오기도 합니다.", tip: "TIP: 드롭 10분 전에 결제 정보와 배송지를 미리 저장해두세요." },
        { icon: "👜", title: "빈티지 & 중고 구매 가이드", body: "빈티지와 중고 패션 시장은 더 이상 입을 수 없는 과거 시즌 아이템이나 단종된 명품을 합리적인 가격에 구할 수 있는 매력적인 쇼핑 방법입니다. 하지만 위조품이나 상태 불량 리스크가 있으므로 현명한 접근이 필요합니다. Grailed과 Vestiaire Collective에서는 판매자 평점과 과거 거래 이력을 반드시 확인하세요. 거래 횟수가 많고 긍정적 평가가 많은 판매자를 우선 선택하는 것이 안전합니다. 상품 사진만으로는 실제 상태를 정확히 판단하기 어려우니, 특히 봉제선, 지퍼, 안감 상태 등 디테일 사진을 추가로 요청하세요. 변색이나 얼룩, 보풀 여부도 꼭 물어보는 것이 좋습니다. 국내에서는 KREAM(크림)의 검수 서비스를 이용하면 정품 여부 걱정 없이 거래할 수 있고, 번개장터와 당근마켓도 빈티지 패션 거래가 활발합니다. 직거래 시에는 반드시 공공장소에서 만나고, 현장에서 상품 상태를 직접 확인하세요. 빈티지 아이템을 구매할 때 가장 주의할 점은 사이즈입니다. 과거 브랜드들의 사이즈 표기는 현대 기준과 다른 경우가 많습니다. 예를 들어, 1990년대 리바이스 501의 32인치는 현재의 32인치보다 넉넉한 핏일 수 있습니다. 따라서 라벨의 사이즈 표기보다 실측(가슴둘레, 어깨너비, 총장 등)을 기준으로 판단해야 합니다. 빈티지 가죽 제품은 특히 관리 상태가 중요합니다. 가죽 크랙이나 금속 부식 여부를 꼼꼼히 확인하고, 구매 후에는 전문 가죽 크리너와 컨디셔너로 관리하면 오래도록 상태를 유지할 수 있습니다. 가격 협상도 중고 거래의 묘미입니다. 대부분의 플랫폼에서 '오퍼' 기능을 통해 희망 가격을 제시할 수 있으며, 시장 가격을 미리 조사한 뒤 합리적인 범위에서 협상하면 좋은 결과를 얻을 수 있습니다.", tip: "TIP: 럭셔리 빈티지는 시리얼 넘버 사진을 반드시 요청하세요." },
        { icon: "👟", title: "스니커즈 리셀 시장 읽기", body: "스니커즈 리셀 시장은 단순한 중고 거래를 넘어 하나의 투자 시장으로 자리 잡았습니다. 현명한 구매를 위해서는 시장의 흐름을 읽는 눈이 필요합니다. StockX와 GOAT는 각 모델의 과거 거래 가격, 거래량, 프리미엄 비율을 그래프로 제공하므로 이를 비교 분석하면 최적의 구매 타이밍을 잡을 수 있습니다. 일반적으로 발매 직후에는 수요가 집중되어 리셀가가 가장 높고, 2~3주가 지나면 시장이 안정되면서 가격이 하락하는 패턴을 보입니다. 한정판이 아닌 일반 발매 모델이라면 서두르지 않는 것이 현명합니다. 반면, 한정 수량으로 발매된 콜라보 모델이나 특별 에디션은 시간이 지날수록 가격이 오르는 경향이 있으므로 초기에 확보하는 것이 유리할 수 있습니다. Kicks Crew는 아시아 지역의 재고를 활용하여 정가 대비 저렴한 가격에 판매하는 경우가 많으니 구매 전 반드시 비교해보세요. 국내에서는 KREAM이 스니커즈 리셀의 중심 플랫폼으로, 검수 서비스를 통해 정품 보장을 제공합니다. 구매 전 사이즈 선택도 신중해야 합니다. 같은 브랜드라도 모델에 따라 핏이 다를 수 있습니다. 예를 들어 나이키 에어포스 1은 보통 반 사이즈 다운을 추천하지만, 뉴발란스 990 시리즈는 정사이즈 또는 반 사이즈 업이 일반적입니다. 리셀 제품은 반품이 어려우므로 사이즈 실수를 하면 다시 리셀해야 하는 번거로움이 생깁니다. 응모(래플) 시스템도 적극 활용하세요. 나이키 SNKRS 앱, 아디다스 컨펌드 앱, 각 편집샵의 래플에 동시에 응모하면 당첨 확률을 높일 수 있습니다. 래플은 무료이므로 관심 있는 모델이 있다면 가능한 모든 채널에서 응모하는 것이 좋습니다. 마지막으로, 스니커즈 커뮤니티(네이버 카페, 레딧 r/sneakers 등)에서 실착 리뷰와 가격 정보를 공유받으면 더욱 현명한 구매 결정을 내릴 수 있습니다.", tip: "TIP: 국내 발매가와 해외 리셀가 + 배송비를 비교한 뒤 결정하세요." },
        { icon: "🌍", title: "글로벌 SPA 현명하게 활용하기", body: "Zara, H&M, Uniqlo 같은 글로벌 SPA(제조·유통 일괄) 브랜드는 빠른 트렌드 반영과 합리적인 가격이 최대 강점입니다. 하지만 무턱대고 구매하면 옷장에 안 입는 옷만 쌓이게 됩니다. 현명하게 활용하는 전략을 알아봅시다. 먼저, 매장과 온라인 재고가 다르다는 점을 기억하세요. 온라인 전용 상품도 많고, 매장에서만 찾을 수 있는 아이템도 있습니다. 마음에 드는 아이템은 온라인에서 먼저 확인하고, 근처 매장에서 실물 핏과 소재감을 체크하는 '쇼루밍' 전략이 효과적입니다. 특히 Zara는 온라인에서 전체 컬렉션을 확인할 수 있지만, 매장에서는 일부만 비치하므로 온라인 사전 조사가 필수입니다. H&M은 기본 라인 외에도 매년 유명 디자이너와의 콜라보 컬렉션을 발매하는데, 이 컬렉션은 발매 당일 매진되는 경우가 대부분이므로 발매일을 미리 체크해두세요. ASOS는 자체 브랜드뿐 아니라 수백 개의 입점 브랜드를 한 곳에서 쇼핑할 수 있는 멀티 플랫폼입니다. 학생 할인 10%와 무료 반품 정책이 큰 강점이며, 사이즈별 모델 착용 사진을 제공하여 온라인 구매 시 사이즈 선택의 불확실성을 줄여줍니다. Uniqlo는 감사제(11월)와 창립기념 세일 시기에 히트텍, 에어리즘 등 베이직 아이템을 대량 구매하는 것이 가성비 최고입니다. 또한 매주 금요일부터 시작되는 기간한정가 세일도 놓치지 마세요. 앱에서 가격 인하 알림을 설정하면 관심 상품이 세일에 들어갈 때 즉시 알림을 받을 수 있습니다. SPA 브랜드에서 구매할 때 가장 중요한 것은 소재 확인입니다. 같은 가격대라도 혼용률에 따라 내구성과 착용감이 크게 달라집니다. 가격표의 소재 정보를 꼭 확인하고, 천연 소재 비율이 높은 제품을 선택하면 오래 입을 수 있습니다. 특히 SPA 브랜드의 겉옷과 팬츠는 가성비가 좋은 반면, 니트류와 속옷은 투자 대비 만족도가 낮을 수 있으니 선택적으로 구매하는 것이 현명합니다.", tip: "TIP: ASOS Premier 가입 시 무제한 익일 배송 혜택을 받을 수 있습니다." }
      ]
    },
    about: {
      title: "DIGGING 소개",
  ...
  en: {
    ...
    guide: {
      ...
      items: [
        { icon: "🇰🇷", title: "Korean Select Shop Tips", body: "Korean platforms like Musinsa, 29CM, and W Concept run major seasonal sales in January and July, with discounts ranging from 30% to 70% on popular brands. App-exclusive coupons often offer better deals than the website, so make sure to install the mobile app and enable notifications for flash sales and limited-time offers. Musinsa operates a daily-rotating time sale and a ranking-based recommendation system that helps you discover trending items at competitive prices. 29CM is known for its editorial approach, featuring curated editor picks with styling suggestions that make it an excellent resource if you need outfit inspiration alongside your purchases. Take advantage of each platform's membership tier system. Musinsa upgrades your membership based on purchase history, unlocking additional discount coupons and free shipping benefits at higher tiers. Similarly, 29CM offers increasing reward rates at each tier level, so concentrating your purchases on one platform pays off in the long run. To avoid sizing mistakes, thoroughly review the measurement details and user-submitted review photos on each platform. Musinsa has a body-type filter feature for reviews, letting you see feedback from people with a similar build. Return policies vary between platforms, so develop a habit of checking them before purchasing. Many offer free returns, but certain brand items or sale merchandise may have return restrictions. Finally, consider buying next-season items just before the season changes. Early-season new arrival coupons are frequently issued, and sizes are still fully stocked, making it much easier to secure the items you want at a better price.", tip: "TIP: Always grab the first-purchase coupon — most offer 10-15% off." },
        { icon: "💎", title: "Luxury Online Shopping Strategy", body: "When buying from international luxury platforms, always factor in customs duties and taxes, as these can add a substantial amount to your total cost. For shipments to Korea, clothing typically incurs approximately 13% customs duty plus 10% VAT on orders exceeding $150 USD. SSENSE and Mytheresa offer up to 70% off during end-of-season sales, with SSENSE being particularly convenient because it uses a DDP (Delivered Duty Paid) model that includes all customs fees at checkout, so there are no surprise charges upon delivery. Farfetch aggregates inventory from hundreds of boutiques worldwide, making it easier to find sold-out pieces and occasionally offering exclusive capsule collections. Monitor exchange rates closely when making purchases, as even a small fluctuation can translate into significant savings on high-value items. It is also worth comparing foreign transaction fees across your credit cards. Some cards offer zero international transaction fees or cashback on overseas purchases, so designating a specific card for luxury shopping can save you money over time. Platforms like Net-a-Porter and MatchesFashion run private sales throughout the season, so subscribe to their newsletters to receive early access. When sizing is uncertain, visit a local department store to try on the same brand beforehand. European brands in particular use different sizing systems from Asian standards, so always cross-reference the size chart carefully. Return policies differ significantly between platforms. SSENSE supports free returns within 30 days, whereas some platforms require customers to cover international return shipping costs. Additionally, purchasing mid-season rather than immediately after new collection launches increases your chances of catching a markdown, so if the purchase is not urgent, patience is a wise strategy. Use the wishlist feature to track price changes on items you are eyeing.", tip: "TIP: Monitor exchange rates — buying when your currency is strong saves significantly." },
        { icon: "🛹", title: "Streetwear Drop Strategy", body: "Streetwear brands like Supreme, Palace, and Stussy release new products through weekly drops at set times, and success requires systematic preparation. Start by following leak accounts on Instagram and Twitter to preview the upcoming week's items. Accounts like Dropsbyjay and Supreme Leaks are reliable sources for early information. Once you know what is dropping, decide on your size and color in advance. During the actual drop, you will need to complete checkout within one to two minutes, leaving no room for deliberation. Save your payment details and shipping address ahead of time, and enable autofill features to speed up the checkout process. Popular items can sell out in under 30 seconds. Kith regularly releases its own in-house designs alongside collaborations with Nike, New Balance, and other brands. These collab products tend to command significant resale premiums. Beams, based in Japan, frequently produces exclusive collaborations with a distinct Japanese aesthetic sensibility. If you miss a drop, resist the urge to immediately buy on the resale market. Initial resale prices are often inflated due to hype, and prices typically stabilize or decrease after one to two weeks as the excitement fades. Keep in mind that international and domestic drop times may differ due to time zones. Desktop checkout tends to be faster than mobile in many cases, and a stable internet connection significantly improves your success rate. Finally, consider the option of lining up for in-person drops at physical stores. The competition can sometimes be less intense than online, and some locations release store-exclusive items that are not available through the website.", tip: "TIP: Save your payment and shipping info 10 minutes before the drop." },
        { icon: "👜", title: "Vintage & Secondhand Guide", body: "The vintage and secondhand fashion market offers a compelling way to find discontinued designer pieces and past-season items at reasonable prices. However, risks such as counterfeits and poor condition require a careful approach. On Grailed and Vestiaire Collective, always check seller ratings and past transaction history. Prioritize sellers with a high number of completed transactions and positive feedback scores for a safer buying experience. Product photos alone are often insufficient to accurately assess condition, so request additional detail shots of stitching, zippers, lining, and any areas prone to wear. Ask specifically about discoloration, stains, or pilling. For domestic transactions, KREAM offers an authentication service that guarantees product legitimacy, and platforms like Bungaejangter are active marketplaces for vintage fashion in Korea. If meeting in person for a direct transaction, always choose a public location and inspect the item thoroughly on the spot. The most critical consideration when buying vintage is sizing. Older garments often use different size standards than modern clothing. For example, a pair of 1990s Levis 501 jeans labeled as 32-inch may fit more loosely than a modern 32-inch waist. Always rely on actual measurements such as chest width, shoulder width, and total length rather than the label size. Vintage leather goods require special attention to condition. Check carefully for leather cracking or metal hardware corrosion, and after purchase, maintain the item with professional leather cleaners and conditioners to preserve its quality over time. Price negotiation is part of the secondhand shopping experience. Most platforms offer an offer feature that allows you to propose your desired price. Research the market value beforehand and negotiate within a reasonable range to increase your chances of a good deal.", tip: "TIP: For luxury vintage, always request serial number photos." },
        { icon: "👟", title: "Reading the Sneaker Resale Market", body: "The sneaker resale market has evolved beyond simple secondhand trading into a full-fledged investment market. Making smart purchases requires understanding how the market moves. StockX and GOAT provide detailed price history graphs, trade volumes, and premium percentages for each model, allowing you to analyze trends and identify the optimal buying window. Resale prices typically peak immediately after release as demand surges, then stabilize and often decline two to three weeks later as the initial hype fades. For general release models that are not limited edition, patience almost always pays off. Conversely, limited-quantity collaboration models and special editions tend to appreciate in value over time, making early acquisition potentially advantageous. Kicks Crew leverages Asian regional inventory to frequently offer prices below retail, so always check their listings before purchasing elsewhere. Domestically, KREAM serves as the central sneaker resale platform in Korea, providing authentication services that guarantee product legitimacy. Size selection requires careful attention as well. Even within the same brand, fit can vary significantly between models. For instance, Nike Air Force 1s typically run half a size large, while New Balance 990 series shoes generally fit true to size or slightly small. Since resale purchases are difficult to return, a sizing mistake means having to resell the pair yourself. Take advantage of raffle systems to secure releases at retail price. Enter draws simultaneously through the Nike SNKRS app, the Adidas Confirmed app, and individual retailer raffles to maximize your chances. Raffles are free to enter, so apply through every available channel for any model you are interested in. Finally, sneaker communities on platforms like Reddit r/sneakers and specialized forums provide invaluable on-feet reviews and pricing intelligence that can help you make better purchasing decisions.", tip: "TIP: Compare domestic retail price vs. overseas resale + shipping before deciding." },
        { icon: "🌍", title: "Making the Most of Global SPA", body: "Global SPA (Specialty retailer of Private label Apparel) brands like Zara, H&M, and Uniqlo offer fast trend adaptation at affordable prices, but buying impulsively can lead to a closet full of unworn clothes. Here is how to shop them strategically. First, remember that in-store and online inventory often differ. Many items are online exclusives, while others can only be found in physical stores. The most effective approach is showrooming: browse the full collection online first, then visit a nearby store to check the actual fit and fabric quality in person. Zara in particular makes its entire collection available online but stocks only a selection in stores, making online research essential. H&M releases annual designer collaboration collections alongside its basic lines. These collaborations typically sell out on launch day, so mark the release dates on your calendar. ASOS functions as a multi-brand platform offering its own label plus hundreds of third-party brands in one place. Its 10% student discount and free returns policy are major advantages, and it provides size-specific model photos that reduce uncertainty when ordering online. Uniqlo delivers the best value during its Anniversary Sale in November and founding anniversary promotions, when staples like Heattech and AIRism are available at significant discounts. Do not overlook the weekly limited-time price drops that begin every Friday. Setting up price drop alerts in the app notifies you the moment an item you are watching goes on sale. The most important factor when shopping SPA brands is checking the fabric composition. Even at the same price point, durability and comfort vary dramatically depending on the material blend. Always read the fabric label and favor items with higher natural fiber content for longevity. Outerwear and trousers from SPA brands tend to offer excellent value for money, whereas knitwear and undergarments may not deliver the same satisfaction relative to their cost, so shop selectively across categories.", tip: "TIP: ASOS Premier membership gives unlimited next-day delivery." }
      ]
    },
    about: {
      title: "About DIGGING",
  ...
  zh: {
    ...
    guide: {
      ...
      items: [
        { icon: "🇰🇷", title: "韩国精选店攻略", body: "Musinsa、29CM、W Concept等韩国精选店平台通常在每年1月和7月进行大规模换季促销，热门品牌商品折扣幅度可达30%至70%。App专属优惠券往往比网页端更多更划算，因此务必安装手机应用并开启推送通知，以便及时获取限时优惠和闪购信息。Musinsa的每日限时特卖和基于销量排行的推荐系统能帮助你以实惠的价格入手潮流单品。29CM以编辑精选栏目著称，将搭配建议与商品推荐相结合，对于需要穿搭灵感的用户尤其实用。同时要充分利用各平台的会员等级制度。Musinsa根据累计消费金额提升会员等级，高等级会员可享受额外折扣券和免运费优惠。29CM同样按等级设定不同的积分返还比例，因此将消费集中在一个平台长期来看更加划算。为避免尺码选择失误，请仔细查看各平台提供的实测数据和用户晒单照片。Musinsa提供按体型筛选评价的功能，让你只看与自己身材相似的买家反馈。退换货政策因平台而异，养成购买前确认的习惯非常重要。虽然许多平台支持免费退货，但部分品牌商品或促销商品可能会限制退换。最后，建议在换季前提前购入下一季的单品。新季初期经常发放新品折扣券，且各尺码库存充足，更容易以优惠价格买到心仪的商品。", tip: "TIP: 首购优惠券必领，大多提供10-15%折扣。" },
        { icon: "💎", title: "奢侈品在线购物策略", body: "从海外奢侈品平台购买时，务必将关税和增值税纳入预算计算。以发往中国为例，进口商品需缴纳关税及增值税，高价值商品的附加费用可能相当可观。SSENSE和Mytheresa在季末大促时最高可享七折优惠，其中SSENSE采用DDP（完税交货）模式，在结账时已包含所有税费，收货时不会产生额外费用，购物体验更为便捷。Farfetch汇集了全球数百家精品店的库存，不仅更容易找到其他平台售罄的商品，还会不定期推出独家胶囊系列。密切关注汇率变动对购物成本有重大影响，汇率波动即使很小，在高价单品上也会产生数百甚至上千元的差异。建议比较各银行信用卡的境外消费手续费，部分银行提供境外消费免手续费或返现优惠，专门办一张用于海外奢侈品购物的信用卡，长期下来能节省不少。Net-a-Porter、MatchesFashion等平台在季中也会举办会员专享特卖，订阅其邮件通知可以获得提前购买权限。尺码拿不准时，建议先到本地专柜试穿同品牌的商品。欧洲品牌的尺码体系与亚洲标准差异较大，务必仔细对照尺码表。各平台的退货政策差异显著：SSENSE支持30天内免费退货，而部分平台需要客户承担国际退货运费。此外，新季系列发布后并不急于购买，等到季中之后入手往往能遇到降价，如果不是特别急需，耐心等待是明智之选。善用愿望清单功能追踪心仪单品的价格变动，也是非常有效的省钱策略。", tip: "TIP: 关注汇率变动，在有利时机购买可节省不少。" },
        { icon: "🛹", title: "街头潮流抢购攻略", body: "Supreme、Palace、Stussy等街头品牌每周定时以Drop（限量发售）形式推出新品，要想提高抢购成功率，系统性的准备工作必不可少。首先在Instagram和Twitter上关注泄露（leak）账号，提前了解下周的发售单品。Dropsbyjay、Supreme Leaks等账号是获取早期信息的可靠来源。确定发售单品后，务必提前锁定尺码和颜色。发售时你需要在一到两分钟内完成结账，根本没有犹豫的余地。提前保存好支付信息和收货地址，并开启自动填充功能以缩短结账时间。热门单品在30秒内售罄的情况屡见不鲜。Kith定期发布自有设计产品以及与Nike、New Balance等品牌的联名款，这些联名产品在转售市场上往往有很高的溢价。Beams则经常推出带有日本独特审美的限定联名系列。如果抢购失败，不建议立即到转售市场高价购入。初期转售价通常因为炒作而虚高，等待一到两周后价格往往会回落并趋于稳定。另外要注意海外与国内的发售时间可能因时差而不同，记得提前确认。在很多情况下电脑端的结账速度比手机端更快，稳定的网络环境也能显著提高抢购成功率。最后，也可以考虑到实体店排队参加线下发售。线下竞争有时不如线上激烈，而且现场还可能出现网上买不到的门店限定单品。", tip: "TIP: 发售前10分钟保存好支付和配送信息。" },
        { icon: "👜", title: "复古与二手购买指南", body: "复古和二手时尚市场为寻找已停产的设计师单品和过季商品提供了以合理价格入手的绝佳途径。但假货和品质不佳的风险始终存在，因此需要谨慎对待。在Grailed和Vestiaire Collective上，务必查看卖家评分和历史交易记录。优先选择成交次数多、好评率高的卖家，交易安全更有保障。仅凭商品照片往往难以准确判断实际状况，建议额外索要走线、拉链、内衬等细节部位以及容易磨损区域的特写照片，同时询问是否存在变色、污渍或起球等问题。在国内市场，KREAM提供鉴定服务保证商品真伪，闲鱼和得物等平台也是活跃的二手时尚交易场所。如果选择线下面交，务必在公共场所见面并当场仔细验货。购买复古单品时最需要注意的是尺码问题。旧款服装的尺码标准往往与现代不同，例如上世纪90年代的Levis 501标注32英寸腰围可能比现在的32英寸更为宽松。因此应以实际测量数据（胸围、肩宽、衣长等）为准，而非标签上的尺码标注。复古皮具需要特别关注保养状况，仔细检查是否有皮面龟裂或五金氧化，购入后使用专业皮革清洁剂和护理油定期保养，可以长久维持其品质。议价是二手交易的一大乐趣。大多数平台都提供出价（Offer）功能，你可以提出自己的期望价格。事先调研市场行情，在合理范围内协商，往往能达成满意的交易。", tip: "TIP: 购买奢侈品复古商品时，务必索要序列号照片。" },
        { icon: "👟", title: "球鞋转售市场解读", body: "球鞋转售市场已从简单的二手交易发展成为一个成熟的投资市场。要想做出明智的购买决策，需要具备解读市场走势的能力。StockX和GOAT提供每款球鞋的历史成交价、交易量和溢价率图表，通过对比分析这些数据可以精准把握最佳购买时机。转售价格通常在发售当天因需求集中而达到峰值，两到三周后随着热度消退价格逐渐回落并趋于稳定。对于非限量的常规发售款，不急于购买是更明智的选择。相反，限量联名款和特别版本往往随着时间推移而升值，尽早入手可能更为有利。Kicks Crew利用亚洲地区的库存资源，经常以低于零售价的价格出售，购买前务必对比查看。在国内，得物和KREAM是球鞋转售的核心平台，提供鉴定服务保障正品。尺码选择也需要格外谨慎，即使是同一品牌的不同型号，版型也可能存在明显差异。例如Nike Air Force 1通常建议小半码，而New Balance 990系列则一般选正码或大半码。转售商品退换困难，尺码选错意味着需要再次转卖，非常麻烦。积极利用抽签（Raffle）系统以零售价购入也是好策略。同时在Nike SNKRS App、Adidas Confirmed App以及各潮店的抽签活动中报名，可以最大化中签概率。抽签是免费参与的，有兴趣的鞋款建议通过所有可用渠道报名。最后，在球鞋社区（如Reddit r/sneakers、虎扑等论坛）获取实穿评测和价格情报，能帮助你做出更加理性的购买决定。", tip: "TIP: 购买前比较国内零售价与海外转售价+运费。" },
        { icon: "🌍", title: "善用全球快时尚", body: "Zara、H&M、优衣库等全球快时尚品牌的最大优势在于快速跟进潮流且价格亲民，但如果盲目购买，衣柜里只会堆满不穿的衣服。以下是聪明购物的策略。首先要记住，线上和线下的库存并不相同。许多单品是线上专属，也有些只能在实体店找到。最有效的方法是先在网上浏览完整系列，再到附近门店检查实物的版型和面料质感，这种'展厅策略'非常实用。Zara尤其适合这种方式，因为网上可以查看全部系列，但门店只陈列部分商品，因此线上预先调研必不可少。H&M除了基础产品线外，每年还会与知名设计师推出联名系列，这类联名通常在发售当天即告售罄，务必提前记下发售日期。ASOS不仅有自有品牌，还是汇集数百个入驻品牌的多品牌平台。学生九折优惠和免费退货政策是其突出优势，同时提供不同尺码模特的上身照片，减少网购时尺码选择的不确定性。优衣库在11月感谢祭和店庆促销期间购入Heattech、AIRism等基础款最为划算。每周五开始的限时特价也不容错过，在App中设置降价提醒，关注的商品一旦降价就能立即收到通知。在快时尚品牌购物时最重要的一点是查看面料成分。同样的价位，不同的面料混纺比例会导致耐穿度和舒适感的巨大差异。务必查看吊牌上的面料信息，天然纤维含量较高的产品更加耐穿。总体而言，快时尚品牌的外套和裤装性价比较高，而针织衫和内衣的性价比可能不尽如人意，因此建议按品类有选择性地购买。", tip: "TIP: 加入ASOS Premier可享无限次日达配送。" }
      ]
    },
    about: {
      title: "关于DIGGING",
  ...
  ja: {
    ...
    guide: {
      ...
      items: [
        { icon: "🇰🇷", title: "韓国セレクトショップ攻略", body: "Musinsa、29CM、W Conceptなどの韓国セレクトショッププラットフォームは、毎年1月と7月に大規模なシーズンオフセールを実施し、人気ブランドの商品が30%から70%オフになります。アプリ限定クーポンはウェブサイトよりも多く、お得な場合がほとんどなので、必ずモバイルアプリをインストールしてプッシュ通知をオンにし、フラッシュセールや限定オファーの情報を逃さないようにしましょう。Musinsaは毎日更新されるタイムセールとランキングベースのレコメンドシステムを運営しており、トレンドアイテムをお得な価格で購入するのに役立ちます。29CMはエディターズピックコーナーでスタイリング提案と合わせて商品を紹介しており、コーディネートのインスピレーションが欲しい方に特におすすめです。各プラットフォームの会員ランク制度も積極的に活用しましょう。Musinsaは購入金額に応じてランクが上がり、上位ランクでは追加の割引クーポンや送料無料特典が付与されます。29CMも同様にランク別でポイント還元率が異なるため、一つのプラットフォームに購入を集中させることが長期的にはお得です。サイズ選びの失敗を避けるには、各プラットフォームの実寸データとユーザーのレビュー写真をしっかり確認してください。Musinsaには体型別のレビューフィルター機能があり、自分に近い体型のレビューだけを絞り込むことができます。返品ポリシーはプラットフォームごとに異なるので、購入前に確認する習慣をつけましょう。無料返品に対応しているところが多いですが、一部のブランド品やセール品は返品が制限される場合があります。最後に、シーズンの変わり目の直前に次シーズンのアイテムを先取り購入する戦略もおすすめです。シーズン初めには新作割引クーポンが頻繁に発行され、サイズも豊富に残っているため、欲しいアイテムをお得に確保しやすくなります。", tip: "TIP: 初回購入クーポンは必ずゲット。ほとんどが10-15%オフです。" },
        { icon: "💎", title: "ラグジュアリーオンライン購入戦略", body: "海外ラグジュアリープラットフォームでの購入時は、関税と消費税を必ず予算に含めて計算しましょう。日本への個人輸入では、商品代金が16,666円を超えると関税と消費税が課されます。衣類の場合、関税率は約10%前後で、さらに消費税10%が加算されるため、実質的な追加コストは無視できません。SSENSEとMytheresaはシーズンオフセールで最大70%オフになり、特にSSENSEはDDP（関税込み）方式を採用しているため、決済時にすべての税金が含まれ、届いた際に追加料金が発生しない便利さがあります。Farfetchは世界中数百のブティックの在庫を一括で閲覧・比較でき、他のプラットフォームで品切れの商品も見つけやすく、限定カプセルコレクションが発売されることもあります。為替レートの変動も要チェックです。円高のタイミングを狙えば、高額商品では数万円の差が生まれることもあります。クレジットカードの海外決済手数料も比較しましょう。一部のカード会社は海外利用手数料無料やキャッシュバック特典を提供しているため、ラグジュアリー直輸入用のカードを一枚用意しておくと長期的に節約できます。Net-a-PorterやMatchesFashionはシーズン中にもプライベートセールを開催するので、ニュースレターを購読して早期アクセスを確保しましょう。サイズに不安がある場合は、国内の百貨店で同じブランドを事前に試着するのが最も確実です。特にヨーロッパブランドはサイズ体系がアジアと異なるため、サイズチャートの丁寧な確認が欠かせません。返品ポリシーはプラットフォームごとに大きく異なり、SSENSEは30日以内の無料返品に対応していますが、国際返送料を購入者が負担するプラットフォームもあります。新コレクション発表直後ではなく、シーズン中盤以降に購入すればマークダウンに出会える確率が高まるので、急ぎでなければ辛抱強く待つのが賢明です。ウィッシュリスト機能を活用して狙いのアイテムの価格変動を追跡するのも有効な戦略です。", tip: "TIP: 為替レートが有利なタイミングを狙って購入すると追加で節約できます。" },
        { icon: "🛹", title: "ストリートウェアドロップ対策", body: "Supreme、Palace、Stussyなどのストリートブランドは、毎週決まった時間に新商品を「ドロップ」形式で販売します。成功率を上げるには、体系的な準備が不可欠です。まずInstagramやTwitterでリーク（leak）アカウントをフォローし、翌週の発売アイテムを事前にチェックしましょう。DropsbyjayやSupreme Leaksなどが代表的な情報源です。発売アイテムを把握したら、サイズとカラーを事前に確定させておきましょう。ドロップ時間中は1〜2分以内に決済まで完了する必要があり、迷っている暇はありません。決済情報と配送先を必ず事前に保存し、オートフィル機能を設定してチェックアウト時間を短縮してください。人気アイテムは30秒以内に完売することも珍しくありません。Kithは自社デザイン製品に加え、Nike、New Balanceなどとのコラボ製品を定期的に発売しており、これらのコラボ製品はリセール市場でプレミアムがつく傾向が強いです。Beamsも日本独自の美意識を活かした限定コラボを頻繁に展開しています。ドロップに失敗した場合、すぐにリセール市場で購入するよりも1〜2週間待つことをおすすめします。発売直後のリセール価格は過熱していることが多く、時間が経てば価格が落ち着くパターンが一般的です。また、海外ドロップと国内ドロップの時間帯が異なる場合があるので時差を確認しましょう。モバイルよりデスクトップの方がチェックアウト速度が速いケースが多く、安定したインターネット環境で試みることが成功率アップにつながります。最後に、実店舗での列並びオフラインドロップも検討する価値があります。オンラインより競争が少ない場合もあり、現場でしか手に入らない限定アイテムが登場することもあります。", tip: "TIP: ドロップ10分前に決済情報と配送先を保存しておきましょう。" },
        { icon: "👜", title: "ヴィンテージ＆中古購入ガイド", body: "ヴィンテージ・中古ファッション市場は、もう手に入らない過去シーズンのアイテムや廃番になったブランド品を手頃な価格で購入できる魅力的なショッピング方法です。ただし偽物や状態不良のリスクがあるため、賢いアプローチが必要です。GrailedやVestiaire Collectiveでは、出品者の評価と過去の取引履歴を必ず確認しましょう。取引回数が多く好評価の出品者を優先的に選ぶことが安全な取引への近道です。商品写真だけでは実際の状態を正確に判断しにくいので、特に縫製線、ジッパー、裏地の状態など細部の写真を追加でリクエストしてください。変色やシミ、毛玉の有無についても必ず質問しましょう。国内ではメルカリやラクマが活発な中古ファッション取引の場となっており、ブランド品はKREAMの鑑定サービスを利用すれば正規品の心配なく取引できます。対面取引の場合は必ず公共の場所で会い、その場で商品の状態を直接確認してください。ヴィンテージアイテムを購入する際に最も注意すべきはサイズです。昔のブランドのサイズ表記は現代の基準と異なるケースが多くあります。例えば1990年代のリーバイス501の32インチは、現在の32インチよりゆったりとしたフィット感かもしれません。そのためラベルのサイズ表記ではなく、実寸（身幅、肩幅、着丈など）を基準に判断すべきです。ヴィンテージレザー製品は特に保管状態が重要です。レザーのひび割れや金属パーツの腐食がないか入念にチェックし、購入後は専門のレザークリーナーとコンディショナーでケアすれば長く状態を維持できます。価格交渉も中古取引の醍醐味です。ほとんどのプラットフォームではオファー機能で希望価格を提示でき、事前に相場を調べた上で合理的な範囲で交渉すれば良い結果を得られるでしょう。", tip: "TIP: ラグジュアリーヴィンテージはシリアルナンバーの写真を必ずリクエスト。" },
        { icon: "👟", title: "スニーカーリセル市場の読み方", body: "スニーカーリセル市場は、単なる中古取引を超えて一つの投資市場として定着しました。賢い購入のためには市場の流れを読む目が必要です。StockXとGOATは各モデルの過去取引価格、取引量、プレミアム率をグラフで提供しているため、これを比較分析すれば最適な購入タイミングを掴めます。一般的に発売直後はリセール価格が最も高騰し、2〜3週間経つと市場が安定して価格が下がるパターンを見せます。限定版でない一般発売モデルなら、焦らないのが賢明です。一方、限定数量でリリースされたコラボモデルや特別エディションは時間の経過とともに値上がりする傾向があるため、早めの確保が有利な場合もあります。Kicks Crewはアジア地域の在庫を活用して定価より安い価格で販売していることが多いので、購入前に必ずチェックしましょう。国内ではスニーカーダンクやKREAMがリセルの中心プラットフォームで、鑑定サービスによる正規品保証を提供しています。サイズ選びも慎重に行う必要があります。同じブランドでもモデルによってフィット感が異なることがあります。例えばナイキ エアフォース1は通常ハーフサイズダウンが推奨されますが、ニューバランス990シリーズは正サイズまたはハーフサイズアップが一般的です。リセル品は返品が難しいため、サイズミスをすると再度リセルする手間が生じます。抽選（ラッフル）システムも積極的に活用しましょう。Nike SNKRSアプリ、Adidas Confirmedアプリ、各セレクトショップのラッフルに同時応募すれば当選確率を高められます。ラッフルは無料なので、気になるモデルがあればすべてのチャネルから応募するのがおすすめです。最後に、スニーカーコミュニティ（Reddit r/sneakers、スニーカーダンクのフォーラムなど）でオンフィートレビューや価格情報を共有してもらえれば、より賢い購入判断ができるでしょう。", tip: "TIP: 国内定価と海外リセール価格+送料を比較してから決めましょう。" },
        { icon: "🌍", title: "グローバルSPAを賢く活用", body: "Zara、H&M、ユニクロなどのグローバルSPA（製造小売）ブランドは、素早いトレンド反映と手頃な価格が最大の強みです。しかし闇雲に購入すると、クローゼットに着ない服が溜まる一方です。賢く活用する戦略をご紹介します。まず、店頭とオンラインで在庫が異なることを覚えておきましょう。オンライン限定商品も多く、店舗でしか見つからないアイテムもあります。最も効果的なのは「ショールーミング」戦略。オンラインでコレクション全体を確認した後、近くの店舗で実際のフィット感や素材感をチェックする方法です。特にZaraはオンラインで全コレクションが閲覧できますが、店舗には一部しか置いていないため、事前のオンラインリサーチが欠かせません。H&Mは基本ラインに加え、毎年有名デザイナーとのコラボコレクションを発売しますが、このコレクションは発売当日に完売するケースがほとんどなので、発売日を事前にチェックしておきましょう。ASOSは自社ブランドだけでなく数百の入居ブランドを一か所でショッピングできるマルチプラットフォームです。学割10%と無料返品ポリシーが大きな強みで、サイズ別のモデル着用写真も提供されているため、オンライン購入時のサイズ選びの不確実性を軽減できます。ユニクロは11月の感謝祭と創業記念セールの時期にヒートテックやエアリズムなどのベーシックアイテムをまとめ買いするのがコスパ最高です。毎週金曜から始まる期間限定価格セールも見逃さないでください。アプリで値下げ通知を設定すれば、気になる商品がセール対象になった瞬間に通知を受け取れます。SPAブランドでの購入で最も重要なのは素材の確認です。同じ価格帯でも混用率によって耐久性と着心地が大きく変わります。素材表示を必ず確認し、天然繊維の比率が高い製品を選べば長く着用できます。総じて、SPAブランドのアウターやパンツはコスパが良い一方、ニット類やインナーは投資対効果が低い場合もあるため、カテゴリー別に選択的に購入するのが賢明です。", tip: "TIP: ASOS Premierに加入すると無制限の翌日配送が利用できます。" }
      ]
    },
    about: {
      title: "DIGGINGについて",
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (visionBtn) {
    visionBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      visionSelector.classList.toggle('open');
      visionBtn.setAttribute('aria-expanded', visionSelector.classList.contains('open'));
    });
  }

  // single global click handler to close all dropdowns
  document.addEventListener('click', () => {
    if (langSelector && langSelector.classList.contains('open')) {
      langSelector.classList.remove('open');
      if (langBtn) langBtn.setAttribute('aria-expanded', 'false');
    }
    if (visionSelector && visionSelector.classList.contains('open')) {
      visionSelector.classList.remove('open');
      if (visionBtn) visionBtn.setAttribute('aria-expanded', 'false');
    }
  });
  ...
  if (visionDropdown) {
    visionDropdown.addEventListener('click', (e) => {
      ...
      if (!opt) return;
      visionSelector.classList.remove('open');
      visionBtn.setAttribute('aria-expanded', 'false');
  ...
  if (hamburger && mobileNav) {
    ...
    mobileNav.addEventListener('click', (e) => {
      if (e.target === mobileNav || e.target.closest('.mobile-nav-close')) {
